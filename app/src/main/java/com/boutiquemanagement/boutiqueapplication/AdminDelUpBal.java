...
public class AdminDelUpBal extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Cursor cursor = db.getData ("SELECT * FROM balouchistan;");
        list.clear ();
        ...
        gridView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, final long id) {
                ...
                AlertDialog.Builder dialog = new AlertDialog.Builder(AdminDelUpBal.this);
                ...
                dialog.setItems(items, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int item) {
                        if (item == 0) {
                            // update
                            Cursor c = AdminAdd.db.getData("SELECT bid FROM balouchistan;");
                            ArrayList<Integer> arrID = new ArrayList<Integer>();
                            while (c.moveToNext()){ arrID.add(c.getInt(0));
                            }
                            showDialogUpdate(AdminDelUpBal.this, arrID.get(position));
                        }
                        else {
                            Cursor c = AdminAdd.db.getData("SELECT bid FROM balouchistan");
                            ArrayList<Integer> arrID = new ArrayList<Integer>();
    ...
    private void showDialogDelete(final int idFood){
        final AlertDialog.Builder dialogDelete = new AlertDialog.Builder(AdminDelUpBal.this);
        ...
        dialogDelete.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                try {
                    AdminAdd.db.deleteDataBal(idFood);
    ...
    private void updateVarietyList(){
        // get all data from sqlite
        Cursor cursor = AdminAdd.db.getData ("SELECT * FROM balouchistan;");
        list.clear ();
        while (cursor.moveToNext ()) {
            Integer id = cursor.getInt (0);
            String name = cursor.getString (1);
